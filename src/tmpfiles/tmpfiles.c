...
static int write_one_file(Item *i, const char *path) {
        ...
        /* Follows symlinks */
        fd = openat(dir_fd, bn, O_NDELAY|O_CLOEXEC|O_WRONLY|O_NOCTTY, i->mode);
        if (fd < 0) {
                if (errno == ENOENT) {
...
static int truncate_file(Item *i, const char *path) {
        ...
        if (fd < 0) {
                ...
                if (fd < 0) {
                        if (errno == ENOENT)
                                return log_error_errno(EROFS, "Failed to create file %s: %m", path);
...
static int create_directory_or_subvolume(const char *path, mode_t mode, bool subvol) {
        ...
        if (subvol) {
                if (btrfs_is_subvol(isempty(arg_root) ? "/" : arg_root) <= 0)
                        ...
                        RUN_WITH_UMASK((~mode) & 0777)
                                r = btrfs_subvol_make_fd(pfd, path);
                }
        }
        ...
        if (r < 0) {
                ...
                if (!k)
                        return log_warning_errno(EEXIST, "\"%s\" already exists and is not a directory.", path);
        ...
        return openat(pfd, basename(path), O_NOCTTY|O_CLOEXEC|O_DIRECTORY);
}
...
static int empty_directory(Item *i, const char *path) {
        ...
        if (r == -ENOENT)
                ...
                return log_debug_errno(0, "Skipping missing file: %s", path);
        if (r < 0)
