...
static int swap_process_new(Manager *m, const char *device, int prio, bool set_flags) {
        ...
        r = swap_setup_unit(m, device, device, prio, false, false, set_flags);
        if (r < 0)
                return r;
        ...
        if (dn && !streq(dn, device))
                swap_setup_unit(m, dn, device, prio, false, false, set_flags);
        ...
        udev_list_entry_foreach(item, first) {
                ...
                swap_setup_unit(m, p, device, prio, false, false, set_flags);
        }
